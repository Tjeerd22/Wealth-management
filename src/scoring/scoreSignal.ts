...
export function scoreSignal(record: NormalizedSignalRecord, lookbackDays: number): NormalizedSignalRecord {
  const sourceQuality = record.source_name === 'afm_mar19' ? 0.72 : 0.78;
  ...
  const signalTypeStrength = record.signal_type === 'substantial_holding_reduction'
    ...
    : record.signal_type === 'pdmr_transaction_unconfirmed'
      ? 0.68
      : 0.45;
  const enrichment = Math.min(1, (record.role ? 0.35 : 0) + (record.enrichment_context ? 0.25 : 0) + (record.company_domain ? 0.1 : 0));

  const score = (sourceQuality * 0.2) + (evidence * 0.22) + (recency * 0.13) + (naturalPerson * 0.22) + (signalTypeStrength * 0.18) + (enrichment * 0.05);
  record.signal_confidence = clamp01(score);

  if (record.signal_type === 'pdmr_transaction_unconfirmed') {
    record.signal_confidence = Math.min(record.signal_confidence, 0.56);
  }
